...
BEGIN
{
    $DB_SQL_username = "xxxxxxxx";
    $DB_SQL_password = "xxxxxxxx!";
    ...
    if(defined $username)
    {
        ...
        my $sth_random123164 = $dbh_random123164->prepare("SELECT password FROM users WHERE email = ?");

        $sth_random123164->execute($email) or die "Couldn't execute statement: $DBI::errstr; stopped";
