...
BEGIN
{
    ...
    if($ref =~ "cookielogin")
    {
        ...
        print "<script>localStorage.clear();window.location.href='index.html';</script>";
        exit;
    }
    ...
    if(defined $username)
    {
        ...
        my $sth_random123164 = $dbh_random123164->prepare("SELECT password, u_cookie, p_cookie, username FROM users WHERE email = ?");

        $sth_random123164->execute($username) or die "Couldn't execute statement: $DBI::errstr; stopped";

        while(my($dbg_password, $dbg_u_cookie, $dbg_p_cookie, $gotten_username) = $sth_random123164->fetchrow_array())
        {
            $got_username = $gotten_username;
